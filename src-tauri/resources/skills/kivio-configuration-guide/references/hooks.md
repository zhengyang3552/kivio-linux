...
2. 工作流修改前读 [协议](protocol.md)。个人配置是完整 `{"enabled":true,"hooks":{...}}`，保存前读取现有文件并保留无关事件与处理器。把候选 JSON 写到工作目录文件，再调用 `kivio_configure {"action":"hooks_save","config_path":"<候选路径>"}`。加载器先验证才覆盖，返回失败要修具体错误。
