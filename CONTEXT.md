...
**子代理**：
由主代理委派工作、拥有独立对话历史的协作对象。完成一次子任务后，仍可沿用其历史继续工作。
_Avoid_: 一次工具调用、临时函数

**子任务**：
委派给子代理的一项具体工作。子任务完成不等于子代理被删除，同一个子代理可以先后承担不同子任务。
_Avoid_: 子代理生命周期

**继续子代理**：
沿用已有子代理的历史，向它交代后续工作。空闲或中断后继续会开始新的执行，不恢复原有进程现场。
_Avoid_: 续聊（该词专指已导入对话）、重新创建子代理

**补充消息**：
向已有子代理提供的新信息或要求。运行中在允许边界接收，空闲时只保留留言，不自动开始工作。
_Avoid_: 自动继续、重启任务

**停止子代理**：
中断指定子代理分支的当前工作，保留历史与已有成果，不撤销已经发生的修改。
_Avoid_: 删除子代理、冻结现场、撤销任务
